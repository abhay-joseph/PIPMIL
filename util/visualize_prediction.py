...
def vis_pred(net, vis_test_dir, classes, device, args: argparse.Namespace):
    ...
    for k, (xs, ys) in enumerate(vis_test_loader): #shuffle is false so should lead to same order as in imgs
        ...
        with torch.no_grad():
            softmaxes, pooled, out, _ = net(xs, inference=True) #softmaxes has shape (bs, num_prototypes, W, H), pooled has shape (bs, num_prototypes), out has shape (bs, num_classes)
